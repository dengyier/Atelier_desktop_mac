...
import { MCP_CONNECTOR_PATH, installConnector, readConnectorState } from './mcp-connectors.mjs'
...
export async function apply(ctx) {
  ...
  ctx.inject(['webServer'], (webCtx) => webCtx.effect(() => {
    let connectorWrite = Promise.resolve()
    const disposeConnectors = webCtx.webServer.register({
      kind: 'exact',
      path: MCP_CONNECTOR_PATH,
      handler: async (req, res) => {
        if (req.method !== 'GET' && req.method !== 'POST') {
          sendJson(res, 405, { error: 'Method not allowed.' })
          return
        }
        if (!isTrustedRequest(req, req.method === 'POST')) {
          sendJson(res, 403, { error: 'Request rejected.' })
          return
        }
        try {
          if (req.method === 'GET') {
            sendJson(res, 200, { connectors: await readConnectorState(home) })
            return
          }
          let body = ''
          for await (const chunk of req) {
            ...
            if (body.length > 8192) {
              sendJson(res, 413, { error: 'Configuration is too large.' })
              return
            }
          }
          const input = JSON.parse(body)
          const result = connectorWrite.then(() => installConnector(home, input))
          connectorWrite = result.catch(() => undefined)
