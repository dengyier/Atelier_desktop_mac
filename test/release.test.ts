...
describe('GitHub release contract', () => {
  ...
  it('uses stable platform-specific artifact names', async () => {
    ...
    expect(packageJson.build.artifactName).toBe('atelier-desktop-${os}-${arch}.${ext}')
    ...
    expect(packageJson.build.nsis.artifactName).toBe(
      'atelier-desktop-windows-${arch}-setup.${ext}'
    )
  ...
  it('does not point the Atelier fork at the upstream DSH update feed', async () => {
    const packageJson = JSON.parse(
      await readFile(path.join(projectRoot, 'package.json'), 'utf8')
    ) as {
  ...
  it('packages an isolated development channel from the current workspace', async () => {
    ...
    expect(developmentConfig.appId).toBe('space.artsmart.atelier.desktop.dev')
    expect(developmentConfig.productName).toBe('Atelier Desktop Dev')
    ...
    expect(developmentConfig.artifactName).toBe('atelier-desktop-dev-${os}-${arch}.${ext}')
    expect(developmentConfig.nsis.artifactName).toBe('atelier-desktop-dev-windows-${arch}-setup.${ext}')
  })
