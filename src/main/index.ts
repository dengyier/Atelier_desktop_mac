...
import { ensureAtelierPreset } from './state/atelier-preset'
import {
...
function configureAppIdentity(): void {
  if (developmentBuild) {
    app.setName('Atelier Desktop Dev')
    app.setPath('userData', join(app.getPath('appData'), 'atelier-desktop-dev'))
    return
  }

  app.setName('Atelier Desktop')
  // Atelier owns a separate Harness home; never mount the user's DSH profile.
  app.setPath('userData', join(app.getPath('appData'), 'atelier-desktop'))
}
...
function ensureTray(): void {
  ...
  tray.setToolTip('Atelier Desktop')
  tray.setContextMenu(
    Menu.buildFromTemplate([
      { label: locale === 'zh' ? '显示 Atelier Desktop' : 'Show Atelier Desktop', click: restoreMainWindow },
      { type: 'separator' },
...
async function maybeImportWebHome(dshHome: string): Promise<void> {
  // This fork starts with an independent profile. Importing a DSH Web home on
  // first launch would copy unrelated sessions and credentials into Atelier.
  if (app.getName().startsWith('Atelier Desktop')) return
...
async function showAbout(window: BrowserWindow): Promise<void> {
  ...
  await dialog.showMessageBox(window, {
    type: 'info',
    title: 'Atelier Desktop',
    message: locale === 'zh' ? '关于 Atelier Desktop' : 'About Atelier Desktop',
    detail: aboutDetail(
      app.getVersion(),
      ...
      ? [
        {
          ...
          submenu: [
            {
              label: isChinese ? '关于 Atelier Desktop' : 'About Atelier Desktop',
              click: () => {
...
async function bootstrap(): Promise<void> {
  ...
  ensureTray()
  const dshHome = join(app.getPath('userData'), 'harness')
  await ensureAtelierPreset(dshHome, desktopResourcePath('atelier-preset'))
  ...
  runtime = new HarnessRuntime({
    ...
    // Keep Atelier's preferred origin separate from DSH Desktop. The runtime
    // reserves a temporary loopback port if another process owns this one.
    preferredPort: 43139 + (developmentBuild ? 1 : 0),
