...
describe('agent preset package transfer', () => {
  ...
  it('keeps a large mode roster searchable, grouped, compact, and links its footer to Atelier', async () => {
    const patch = await readFile(
      patchPath('@deepseek-ai/dsh-client-ui-agent-preset'),
      'utf8'
    )
    ...
    expect(patch).toContain('id: ATELIER_WEBSITE_ID')
    expect(patch).toContain('visitAtelierWebsite: "访问 Atelier 官网…"')
    expect(patch).toContain('visitAtelierWebsite: "Visit Atelier website…"')
    expect(patch).toContain('if (id === ATELIER_WEBSITE_ID)')
    expect(patch).toContain('window.open("https://artsmart.space/", "_blank", "noopener,noreferrer")')
  })
