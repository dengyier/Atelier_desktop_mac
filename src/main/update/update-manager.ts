...
export function registerUpdateHandlers(): void {
  ...
  ipcMain.handle('updates:list-versions', () =>
    supportsUpdates() ? fetchAvailableReleases(app.getVersion()) : []
  )
...
function supportsUpdates(): boolean {
  // Atelier has no signed release feed yet. Never install DSH Desktop updates
  // over this independently named application.
  if (app.getName?.().startsWith('Atelier Desktop')) return false
